...
const opinionModel=require("../model/opinionModel.js");
...
router.get('/getData',(req,res)=>{
	...
	let se={}
	opinionModel.find({$or:[{"username":{$regex:content}},{"opincontent":{$regex:content}},{"pushtime":{$regex:content}}]}).sort({'_id':style})
	.then((data)=>{
		// console.log(11)
		sum=data.length
		// let num = Math.ceil(sum/yu)*1;
		// console.log(sum,97)
		return opinionModel.find({$or:[{"username":{$regex:content}},{"opincontent":{$regex:content}},{"pushtime":{$regex:content}}]}).sort({'_id':style}).limit(yu).skip((target-1)*yu)
	})
	.then((data2)=>{
		// console.log(22)
		se={sum,data2}
...
router.post('/delMore',(req,res)=>{
	...
	let id=0
	opinionModel.deleteMany({ _id:{ $in:ss}})
	.then((data)=>{
		if(data.length>0){
			res.send(sendData(0,'删除成功',data))
		}
...
router.post('/update2',(req,res)=>{
	...
	// .then((data)=>{
		// if(data.length>0){
		opinionModel.updateOne({_id:gid},{title,Ltitle,style,main,img})
